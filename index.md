...
- [Evaluation Harness Lab](labs/02-evaluation-harness): Build a rubric-driven scorer to catch regressions.
- [Brand Voice & UX Lab](labs/03-voice-and-ux-lab): Stress-test tone, refusals, and persona overlays.
...
## Experience Features to Add Next

- **Settings flyout.** Allow visitors to pick accent colors, typography scale, and spacing density. Persist choices and export them as a small design-token JSON so teams can mirror the look in their own docs.
- **Audience presets.** Quick toggles for "executive review", "builder deep-dive", and "compliance" that change lede copy, surface acceptance criteria, and reveal or hide rationale blocks.
- **Pattern playground.** Inline code blocks that can toggle between "teaching" and "delivery" modes, revealing failures, redlines, and the final production prompt side-by-side.
- **Accessibility and focus mode.** A light theme, focus outlines, and reading-width toggle to keep the experience inclusive without sacrificing the premium brand.
- **Handoff kits.** Buttons that package the current page as PDF/Markdown plus a short QA checklist—ideal for client approvals and internal briefings.
