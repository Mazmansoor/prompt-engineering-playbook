...
- **Runbook pane.** Attach a right-rail that tracks tool calls, errors, and rationale so reviewers can quickly audit behavior without digging through raw logs.
- **Settings page hooks.** Let operators tune max tool calls, retry counts, and fallback policies from a settings UI, then serialize those preferences into the prompt to keep behavior aligned.
- **Observability events.** Emit structured events for each tool call (inputs, latency, result class) so the evaluation cockpit can chart regressions over time.
- **Preset packs.** Bundle configurations for "high-speed triage", "compliance-heavy investigations", and "cost-sensitive research" to give teams confidence about which levers change experience and spend.
